...
((day) => {
  ...
  fs.copyFile(path.resolve(__dirname, '../answers/template.js'), path.resolve(__dirname,`../answers/day_${day}.js`))
    .then(() => {
  ...
  // Once input is downloaded, save to file then run test
    .then((response) => {
      ...
      fs.writeFile(`./input/day_${day}.txt`, input)
        .then(()=> {
          console.log(`Input for day ${day} has bee downloaded to /input/day_${day}.txt`);
        })
        .catch((err)=>{
          console.log(err);
        });
