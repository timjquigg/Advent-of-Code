export const runtest = (inputStr: string) => {
  ...
  const crates = input.slice(
    0,
    input.findIndex((el) => el === "")
  );
  const moves = input.slice(crates.length + 1);

  const crateObj: { [key: string]: string[] } = {};
  crates.forEach((el) => {
    let j = 0;
  ...
  // Part 1:
  const part1Crates: { [key: string]: string[] } = JSON.parse(
    JSON.stringify(crateObj)
  );
