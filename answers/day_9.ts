...
const moveHead = (input: string[][]): number[][] => {
  const start = [0, 0];
  const history = [start];
...
const moveTail = (headPositions: number[][]): number[][] => {
  const start = [0, 0];
  ...
  for (let i = 0; i < headPositions.length; i++) {
    ...
    if (
      ...
      (Math.abs(diffY) > 1 && Math.abs(diffX) >= 1)
    ) {
...
const getUnique = (positions: number[][]): number[][] => {
  const uniqueObj: { [key: string]: number[] } = {};
...
export const runtest = (inputStr: string) => {
  ...
  console.log(
    `Number of unique locations visited by the tail: ${oneTailUniquePositions.length}`
  );

  // Part 2
  const nineTailsUniqueTailPositions = getUnique(tailPositions.slice(-1)[0]);
  console.log(
    `Number of unique locations visited by the tail: ${nineTailsUniqueTailPositions.length}`
  );
};
